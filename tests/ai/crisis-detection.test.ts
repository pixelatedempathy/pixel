...
describe('crisisDetectionService', () => {
  ...
  beforeEach(async () => {
    ...
    const serviceModule = await import('../../lib/ai/services/crisis-detection.js');
  ...
  describe('detectCrisis', () => {
    ...
    it('should handle batch processing correctly', async () => {
      ...
      vi.mocked(mockAIService.createChatCompletion)
        .mockResolvedValueOnce({ content: JSON.stringify({ score: 0.9 }) } as AICompletion)
        .mockResolvedValueOnce({ content: JSON.stringify({ score: 0.1 }) } as AICompletion)
        .mockResolvedValueOnce({ content: JSON.stringify({ score: 0.7 }) } as AICompletion);
