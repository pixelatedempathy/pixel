...
/**
 * Breach data access class
 */
export class BreachDataService {
  /**
   * Store a new security breach
   */
  static async createBreach(breach: SecurityBreach): Promise<void> {
    try {
   ...
   */
  static async getBreachesSince(date: Date): Promise<SecurityBreach[]> {
    try {
   ...
   * Get breach by ID
   */
  static async getBreachById(id: string): Promise<SecurityBreach | null> {
    try {
      const { data, error } = await supabase
        .from('security_breaches')
        .select('*')
   ...
   */
  static async deleteBreach(id: string): Promise<void> {
    try {
      const { error } = await supabase
        .from('security_breaches')
        .delete()
