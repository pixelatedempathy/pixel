...
/**
 * Class for calculating comprehensive notification effectiveness metrics
 */
export class NotificationEffectiveness {
  /**
   * Calculates delivery effectiveness based on notification success rates
   */
  private static calculateDeliveryScore(breach: Breach): number {
   ...
   */
  private static calculateTimingScore(breach: Breach): number {
   ...
   */
  private static calculateAcknowledgmentScore(breach: Breach): number {
    if (!breach.notifications) {
      return 0
    }
   ...
   */
  private static calculateComplianceScore(breach: Breach): number {
    if (
   ...
   */
  static async calculate(breaches: Breach[]): Promise<EffectivenessMetrics> {
    ...
    try {
      ...
      for (const breach of breaches) {
        ...
        totalDeliveryScore += this.calculateDeliveryScore(breach)
        totalTimingScore += this.calculateTimingScore(breach)
        totalAckScore += this.calculateAcknowledgmentScore(breach)
        totalComplianceScore += this.calculateComplianceScore(breach)
      }
  ...
  static async calculateDaily(
    ...
    return this.calculate(dailyBreaches)
  }

  /**
   * Returns empty metrics structure for when no breaches are provided
   */
  private static getEmptyMetrics(): EffectivenessMetrics {
    return {
      overall: 0,
      delivery: 0,
      timing: 0,
