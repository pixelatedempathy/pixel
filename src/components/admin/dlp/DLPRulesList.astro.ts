...
function handleRuleUpdated(event: CustomEvent) {
  ...
  console.log(`Rule updated: ${name} (${id}) is now ${isActive ? 'active' : 'inactive'}`);
}
...
const handleRuleUpdatedListener = (e: Event) => handleRuleUpdated(e as CustomEvent);
const handleRuleDeletedListener = (e: Event) => handleRuleDeleted(e as CustomEvent);
