...
setup_environment() {
	...
	print_color "$BLUE" "🔧 Setting up environment ${n $win}dow..."
...
create_agent_session() {
	print_color "$GREEN" "🚀 Creating AI Agents tmux sessio${: $SESSION_N}AME"
...
attach_session() {
	print_color "$BLUE" "📎 Attaching to existing sessio${: $SESSION_N}AME"
...
show_status() {
	if session_exists; then
		print_color "$GREEN" "✅ Session${'$SESSION_NA}ME' is running"
		print_color "$CYAN" "Windows:"
		tmux list-windows -t "${SESSION_NAME}" -F "   #{window_index}: #{window_name} - #{window_flags}"
	else
		print_color "$RED" "❌ Session${'$SESSION_NA}ME' is not running"
	fi
}
...
"create")
	if session_exists; then
		print_color "$YELLOW" "⚠️ Sessi${n '$SESSION_}NAME' already exists!"
		show_status
	else
		create_agent_session
	fi
