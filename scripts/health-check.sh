...
check_http() {
	...
	echo -n "🔍 Checki${g $service_n}ame... "

	if response=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 "${url}" 2>/dev/null); then
		if [[ "${response}" = "${expected_status}" ]]; then
			echo -e "${GREEN}✅ Healthy (HTT${ $respon}se)${NC}"
			return 0
		else
			echo -e "${YELLOW}⚠️  Warning (H${TP $resp}onse)${NC}"
			return 1
		fi
	else
...
check_docker_service() {
	...
	echo -n "🐳 Checking Docker servi${e $service_n}ame... "
