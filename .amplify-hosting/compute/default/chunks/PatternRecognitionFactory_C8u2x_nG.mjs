;!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="c321c933-19bf-4e9e-8d63-97ea6fa3674f",e._sentryDebugIdIdentifier="sentry-dbid-c321c933-19bf-4e9e-8d63-97ea6fa3674f")}catch(e){}}();import { c as createBuildSafeLogger } from './build-safe-logger_TXSN-RsD.mjs';
...
async function createPatternRecognitionFHEService(config) {
  try {
    ...
    if (useMock) {
      ...
      const { SealPatternRecognitionService } = await import('./seal-pattern-recognition_C4psSrbB.mjs');
...
class ConcretePatternRecognitionService {
  ...
  async analyzeRiskFactorCorrelations(clientId, analyses) {
    try {
      ...
      const results = correlations.map((correlation) => ({
        primaryFactor: correlation.factor || "anxiety",
        correlatedFactors: correlation.correlations?.map((c) => ({
          factor: c.factor || "unknown",
          correlation: c.value || 0,
          confidence: c.confidence || 0.7,
          pValue: c.pValue || 0.05,
          effectSize: "medium"
        })) || [],
        timeFrame: {
          start: correlation.startDate || /* @__PURE__ */ new Date(),
          end: correlation.endDate || /* @__PURE__ */ new Date(),
          duration: 30
          // days
        },
        severity: correlation.riskLevel || "medium",
        actionRequired: correlation.requiresAction || false,
        recommendations: correlation.recommendations || ["Monitor this correlation"],
        statisticalMetrics: {
          sampleSize: analyses.length,
          pearsonCorrelation: correlation.pearson || 0.5,
          spearmanCorrelation: correlation.spearman || 0.5,
          kendallTau: correlation.kendall || 0.4,
  ...
  async analyzeLongTermTrends(clientId, startDate, endDate) {
    try {
      ...
      const results = trendPatterns.map((pattern) => ({
        id: pattern.id || `trend-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
        type: pattern.type || "emotional",
        confidence: pattern.confidence || 0.7,
        startDate: pattern.startDate || startDate,
        endDate: pattern.endDate || endDate,
        description: pattern.description || "Long-term trend detected",
        indicators: pattern.indicators || ["mood", "anxiety", "sleep"],
        algorithmicAnalysis: {
          trendDirection: "increasing",
          trendStrength: pattern.strength || 0.6,
          linearRegression: {
            slope: 0.05,
            intercept: 0.2,
            rSquared: 0.75,
